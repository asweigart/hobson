...
class Window:
    """
    A single tkinter window that contains HobsonPy UI widgets such as buttons, text boxes, and a general surface to draw text characters on.
    ...
    TODO? No. This is so that the widgets don't have to be responsive to window resizes. You can resize the tkinter window though. This is not advised.
    ...
    Is there support for non-monospace fonts?
    No.
    ...
    No.
    """
    def __init__(self, width, height, title='', fg='white', bg='#272822', fontsize=10):
        """
        ...
        self.width = width # width is in number of cells, not pixels
        self.height = height # height is in number of cells, not pixels
        ...
        if type(fontsize) != int:
            raise TypeError('Parameter `fontsize` must be of type int, not %s.' % (type(fontsize)))
        if fontsize <= 0:
            raise ValueError('Parameter `fontsize` must be greater than 0, not %s.' % (fontsize))

        self._widgets = [] # a list of all the widgets added to the Window. This is used to check for overlapping widgets.


    def draw_line(self, startx, starty, endx, endy, char='*', fg=None, bg=None):
        """
        ...
        if len(start) != 2:
            raise ValueError('Parameter `start` must be a sequence of two numbers.')
        if type(start[0]) not in (int, float):
            raise TypeError('X position of parameter `start` must be a float or int, not a %s.' % (type(start[0])))
        if type(start[1]) not in (int, float):
            raise TypeError('Y position of parameter `start` must be a float or int, not a %s.' % (type(start[1])))

        if len(end) != 2:
            raise ValueError('Parameter `end` must have two numbers.')
        if type(end[0]) not in (int, float):
            raise TypeError('X position of parameter `end` must be a float or int, not a %s.' % (type(end[0])))
        if type(end[1]) not in (int, float):
            raise TypeError('Y position of parameter `end` must be a float or int, not a %s.' % (type(end[1])))
        ...
        if fg is not None:
            ...
            fg = self.fg # use the window's fg color
        if bg is not None:
            bg = hex_color(bg)
        else:
            bg = self.bg # use the window's bg color


        pass


    def draw_rect(self, top, left, width, height, char='*', fg=None, bg=None, filled=False):
        """
        Note that the draw_*() functions only draw to the screen once and will not automatically redraw themselves later.
        ...
        if len(topleft) != 2:
            raise ValueError('Parameter `topleft` must contain two integer positions.')
        if type(topleft[0]) != int:
            raise TypeError('Item 0 of parameter `topleft` must be an int, not %s.' % (type(topleft[0])))
        if type(topleft[1]) != int:
            raise TypeError('Item 1 of parameter `topleft` must be an int, not %s.' % (type(topleft[1])))
    ...
    def draw_fill(self, char=' ', fg=None, bg=None):
        """
        Note that the draw_*() functions only draw to the screen once and will not automatically redraw themselves later.
    ...
    def draw_box(self, top, left, width, height, title='', fg=None, bg=None, border=SINGLE):
...
class Cell:
    def __init__(self, char, fg=None, bg=None, click=None, dblclick=None, mouseover=None, mouseout=None, contextmenu=None, focus=None, blur=None):
        self.char = char
        self.fg = fg
        self.bg = bg
        self.click = click
...
class ProgressBar(Widget):
    def __init__(self, x, y, value=0, min=0, max=100, bar_char='#', empty_char='.', orientation=HORIZONTAL):
        pass


class Spinner(Widget):
    """
    Can I adjust the speed of the spinner animation?
    No.

    Can progress bars be larger than a single character?
    No.
    """
